...
(function () {
  ...
  (function () {
    ...
    var Serverless = (function() {
      ...
      // Node-style callback on top of the promise. Attaching the handlers here
      // marks a rejection as handled, so a callback user who ignores the return
      // value doesn't get an "unhandled rejection" for every failed call.
      ...
      function parseEnvelope(name, response) {
        return response.text().then(function(text) {
          ...
          if (!data.ok) {
            throw serverlessError(data.error || 'Endpoint ' + name + ' failed (HTTP ' + response.status + ')', response.status, data.parameters);
          }
          return data.result;
        });
      }
