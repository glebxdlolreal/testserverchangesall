...
(function () {
    ...
    TAxisY.prototype = {
        ...
        calcAxisData: function (y1Name, y2Name) {
            ...
            var yRealStep = units.TUtils.niceNum((y2 - y1) / this.opts.settings.Y_AXIS_RANGE, true);
            var yRealStart = y1;
        ...
        updateAxisState: function (y1Name, y2Name, numName, baseData, leftData, rightData) {
            ...
            for (var i = 0; i <= linesCount; i++) {
                var p = 1e12;
                var numReal = Math.round((baseData.yRealStart + baseData.yRealStep * i) * p) / p;
                var numRealLeft = Math.round((leftData.yRealStart + leftData.yRealStep * i) * p) / p;
                var numRealRight = Math.round((rightData.yRealStart + rightData.yRealStep * i) * p) / p;
                ...
                var numDisplayRightStr = formatter(Math.max(numRealRight, 0), rightData.yRealStep, true/*, this.opts.state['e_0']*/);
...
(function () {
    ...
    var TChart = function (opts) {
        ...
        var xDatesEmpty = opts.data.xRangeFormatter == statsFormatEmpty;
        ...
        this.data = {
            ...
            yCustomTooltipFormatters: opts.data.yCustomTooltipFormatters,
            ...
            tooltipSort: opts.data.tooltipSort,
            subchart: {
        ...
        opts.data.columns.forEach(function (item, ind) {
            ...
            if (tp !== 'x') {
                ...
                var ctf = opts.data.yCustomTooltipFormatters ? opts.data.yCustomTooltipFormatters[id] : null;
                this.data.ys.push({
            ...
            if (this.graphStyle === 'scatter' && opts.data.scatter && !this._scatterPrepared) {
                this._scatterPrepared = true;

                this.data.scatter = opts.data.scatter;
                var sData = this.data.scatter;

                var globXMin = Number.MAX_VALUE;
                var globXMax = -Number.MAX_VALUE;
                var hasPoints = false;

                for (var sid in sData.series) {
                    var pts = sData.series[sid].points;
                    if (pts && pts.length) {
                        pts.sort(function (a, b) {
                            return a.x - b.x;
                        });

                        if (pts[0].x < globXMin) globXMin = pts[0].x;
                        if (pts[pts.length - 1].x > globXMax) globXMax = pts[pts.length - 1].x;

                        sData.series[sid].xMin = pts[0].x;
                        sData.series[sid].xMax = pts[pts.length - 1].x;
                        hasPoints = true;
                    }
                }

                if (hasPoints) {
                    this.state.xMainMin = globXMin;
                    this.state.xMainMax = globXMax;
                    this.state.xgMin = globXMin;
                    this.state.xgMax = globXMax;
                    this.state.xg1 = globXMin;
                    this.state.xg2 = globXMax;
                    var defZoom = this.getDefaultZoom({
                        x1: globXMin,
                        x2: globXMax,
                        xg1: globXMin,
                        xg2: globXMax,
                    ...
                    this.state.x1 = defZoom.x1;
                    this.state.x2 = defZoom.x2;
                    this.state.xg1Ind = 0;
                    this.state.xg2Ind = Math.max((this.data.x ? this.data.x.length : 1) - 1, 0);
                }
            }
        }.bind(this));

        units.TUtils.triggerEvent('chart-update-zoom', {
            x1: this.state.x1,
            x2: this.state.x2,
            target: opts.container.parentElement,
        });
        ...
        var reducedRange = {isReduced: false};
        if (this.graphStyle !== 'scatter') {
            reducedRange = this.reduceGlobalRange({});
        }
    ...
    TChart.prototype = {
        ...
        getScatterYMinMax: function (x1, x2) {
            var yMin = Number.MAX_VALUE;
            var yMax = -Number.MAX_VALUE;
            var state = this.state;
            var series = this.data.scatter.series;
            ...
            function lowerBoundX(arr, x) {
                var lo = 0, hi = arr.length; // hi is exclusive
                while (lo < hi) {
                    var mid = (lo + hi) >> 1;
            ...
            this.data.ys.forEach(function (item, ind) {
                if (!state['e_' + ind]) return;

                var sData = series[item.id];
                if (!sData || !sData.points || !sData.points.length) return;

                if (sData.xMin > x2 || sData.xMax < x1) return;

                hasVisible = true;

                if (sData.blocks) {
                    var bLen = sData.blocks.length;
                    for (var i = 0; i < bLen; i++) {
                        var b = sData.blocks[i];

                        // no overlap
                        if (b.xMax < x1) continue;
                        if (b.xMin > x2) break;

                        // fully inside range
                        if (b.xMin >= x1 && b.xMax <= x2) {
                            expand(b.yMin);
                            expand(b.yMax);
                            continue;
                        }

                        // bin src works bcs it's already sorted by x
                        var ptsB = b.points;
                        var start = lowerBoundX(ptsB, x1);
                        for (var k = start; k < ptsB.length; k++) {
                            var px = ptsB[k].x;
                            if (px > x2) break;
                            expand(ptsB[k].y);
                        }
                    ...
                    var pts = sData.points;
                    var start = lowerBoundX(pts, x1);
                    for (var i2 = start; i2 < pts.length; i2++) {
                        var px2 = pts[i2].x;
                        if (px2 > x2) break;
                        expand(pts[i2].y);
                    }
            ...
            if (!hasVisible || yMin === Number.MAX_VALUE) {
                return {min: 0, max: 10};
            }
        ...
        getYMinMax: function (x1, x2, isMini, fitTo, useSaved) {
            ...
            if (this.graphStyle == 'scatter') {
                return this.getScatterYMinMax(x1, x2);
            }
            ...
            function proceedMinMax(yMin, yMax, ind, refRange) {
                if (yMin == Number.MAX_VALUE) {
                    ...
                    // ALWAYS use roundRange - it handles nice numbers
        ...
        onHandleMove: function (x1, x2, tp, firstMove) {
            ...
            units.TUtils.triggerEvent('chart-update-zoom', {
                x1: x1,
                x2: x2,
                target: this.opts.container.parentElement,
            });
        ...
        toggleZoom: function (enabled, dt, data) {
            ...
            if (this.specialZoomTransition) {
                ...
                if (!this.slaveChart) {
                    ...
                    clonedData.yCustomTooltipFormatters = data.yCustomTooltipFormatters;
                    ...
                    clonedData.tooltipSort = data.tooltipSort != undefined ? data.tooltipSort : this.data.tooltipSort;
            ...
            if (enabled) {
                ...
                if (data) {
                    this.data.details = {
                        ...
                        yCustomTooltipFormatters: data.yCustomTooltipFormatters,
    ...
    TComposer.prototype = {
        ...
        renderInner: function (groups) {
            ...
            if (groups.top) {
                ...
                if (this.opts.graphStyle == 'line' || this.opts.graphStyle == 'step' || this.opts.graphStyle == 'scatter' || (this.opts.graphStyle == 'area' && zoomMorph > 0) || (this.opts.data.slave && state.slaveVisibility < 1) || (this.opts.data.master && state.masterVisibility < 1)) {
                    if (pieChartAnimated) {
...
(function () {
    ...
    TFade.prototype = {
        onResize: function () {
            ...
            var g_color = getComputedStyle(this.opts.$canvas).getPropertyValue('--tchart-fade-color');
            if (!g_color) {
                g_color = this.isDarkMode ? 'rgba(36,47,62,1)' : 'rgba(255,255,255,1)';
            }

            if (this.opts.graphStyle != 'area') {
                var gradientTop = this.ctxFadeTop.createLinearGradient(0, 0, 0, dimsTop.h * dpi);
                gradientTop.addColorStop(0, g_color);
                gradientTop.addColorStop(1, g_color);
                ...
                if (this.opts.graphStyle != 'bar') {
                    ...
                    gradientBottom.addColorStop(0, g_color);
                    gradientBottom.addColorStop(1, g_color);
...
(function () {
    ...
    var TTip = function (opts) {
        ...
        this.tooltipSortOrder = (function (ts) {
            if (ts === true || ts === 'desc') return 'desc';
            if (ts === 'asc') return 'asc';
            return null;
        })(opts.data.tooltipSort);
    ...
    TTip.prototype = {
        ...
        onTipClick: function (e) {
            // scatter: click navigates to point.href, zoom disabled
            if (this.opts.graphStyle === 'scatter') {
                // prefer last hovered point (set by renderScatterTooltip)
                var pt = this.lastScatterPoint;

                // fallback, try to pick again
                if (!pt && this.dx != null && this.dy != null) {
                    var dimsTip = this.opts.state.dims.tip;
                    var dimsGraph = this.opts.state.dims.graph;
                    var gx = this.dx - dimsGraph.l;
                    var gy = this.dy - dimsTip.t;
                    var picked = this.opts.chart.graph.pickPoint(gx, gy);
                    pt = picked && picked.point;
                }
                ...
                if (pt && pt.href) {
                    ...
                    // hide tooltip before navigation so UI doesn't stick
                    this.toggle(false, true);

                    // pop blocker may kill it
                    try {
                        window.open(pt.href, '_blank', 'noopener,noreferrer');
                    } catch (error) {
                    }
                }
            ...
            var errorDetailCallback = function (val) {
                ...
                if (val === 'default' &&
                    self.opts.additional.onClick &&
                    self.opts.data.hasDetail &&
                    self.opts.graphStyle == 'area') {
                    self.toggle(false, true);
                    self.opts.additional.onClick(true, x);
                } else {
        ...
        getTp: function (x, y, isTouch) {
            ...
            } else if (this.opts.graphStyle == 'scatter') {
                var dimsTip = this.opts.state.dims.tip;
                var dimsGraph = this.opts.state.dims.graph;

                var gx = x - dimsGraph.l;
                var gy = y; // already relative to graph top

                if (gy < 0 || gy > dimsTip.h) return '';
                if (gx < 0 || gx > dimsGraph.w) return '';

                var picked = this.opts.chart.graph.pickPoint(gx, gy);

                // cache for cursor (trackMouse handler) and click fallback
                this._scatterLastPick = picked;
                this._scatterLastPickGX = gx;
                this._scatterLastPickGY = gy;
        ...
        onMouseMove: function (e) {
            ...
            var dimsTip = this.opts.state.dims.tip;
            var tp = this.getTp(dx, dy - dimsTip.t, false);

            this.onMouseLeave();

            if (this.opts.graphStyle === 'scatter') {
                // pointer only on clickable dots
                var dimsGraph = this.opts.state.dims.graph;
                var gx = dx - dimsGraph.l;
                var gy = dy - dimsTip.t;

                // cached pick if it == current coords, otherwise pick again
                var picked = null;
                if (this._scatterLastPickGX === gx && this._scatterLastPickGY === gy) {
                    picked = this._scatterLastPick;
                } else {
                    picked = this.opts.chart.graph.pickPoint(gx, gy);
                    this._scatterLastPick = picked;
                    this._scatterLastPickGX = gx;
                    this._scatterLastPickGY = gy;
                }

                if (picked && picked.point && picked.point.href) {
                    this.$canvas.classList.add('tchart--graph-canvas__tip-pointer');
                }
            } else {
        ...
        toggle: function (enabled, shiftHide) {
            ...
            if (enabled && !this.shown) {
                ...
                // scatter: click is href, not zoom
                if (this.opts.graphStyle !== 'scatter' &&
                    this.opts.data.hasDetail &&
                    !(this.opts.state.zoomMode || this.opts.state.zoomModeSpecial)) {
                ...
                if (this.$line && this.opts.graphStyle !== 'scatter') {
                    this.$line.classList.add('tchart--line__visible');
                }
            ...
            if (!enabled && this.shown) {
                ...
                // scatter: clear hover highlight when tooltip is hidden
                if (this.opts.graphStyle === 'scatter' &&
                    this.opts.chart && this.opts.chart.graph && this.opts.chart.graph.clearHover) {
                    this.opts.chart.graph.clearHover();
                }
                ...
                if (this.$line && this.opts.graphStyle !== 'scatter') {
                    this.$line.classList.remove('tchart--line__visible');
                }
        ...
        render: function (params) {
            ...
            if (opts.graphStyle == 'area' && state.zoomMode) {
                ...
                return;
            } else if (opts.graphStyle == 'scatter') {
                this.renderScatterTooltip(params);
                return;
            } else {
                if (this.pieLabel) {
            ...
            if (this.prevXInd != xInd || !params.isMove) {
                ...
                opts.data.ys.forEach(function (item, ind) {
                    if (opts.state['e_' + ind] && !isNaN(item.y[xInd])) {
                        this.labels[ind].$valueText.nodeValue = (item.ctf || formatter)(item.y[xInd]);
                ...
                // sort tooltip list
                if (this.tooltipSortOrder) {
                    var sorted = [];
                    for (var si = 0; si < this.labels.length; si++) {
                        if (this.opts.state['e_' + si] && !isNaN(this.opts.data.ys[si].y[xInd])) {
                            sorted.push({ind: si, val: this.opts.data.ys[si].y[xInd] || 0});
                        }
                    }
                    var order = this.tooltipSortOrder;
                    sorted.sort(function (a, b) {
                        if (a.val === b.val) return a.ind - b.ind;
                        return order === 'desc' ? (b.val - a.val) : (a.val - b.val);
                    });
                    for (var s = 0; s < sorted.length; s++) {
                        this.$tipScroller.appendChild(this.labels[sorted[s].ind].$row);
                    }
                    for (var si2 = 0; si2 < this.labels.length; si2++) {
                        if (!(this.opts.state['e_' + si2] && !isNaN(this.opts.data.ys[si2].y[xInd]))) {
                            this.$tipScroller.appendChild(this.labels[si2].$row);
                        }
                    }
                }
        ...
        renderScatterTooltip: function (params) {
            var opts = this.opts;
            var settings = this.opts.settings;
            var dims = opts.state.dims.tip;
            var pTop = settings.PADD[0];
            var pBottom = settings.PADD[2];
            var pLeft = settings.PADD[3];
            var pRight = settings.PADD[1];

            var dimsGraph = opts.state.dims.graph;

            // convert pointer from canvas-local to graph-local
            var gx = this.dx - dimsGraph.l;
            var gy = (this.dy - dims.t); // graph-local already (dims.t == graph.t)

            var picked = opts.chart.graph.pickPoint(gx, gy);
            if (!picked) {
                opts.chart.graph && opts.chart.graph.clearHover && opts.chart.graph.clearHover();
                return;
            }

            // highlight hovered dot (overlay canvas, no full redraw)
            opts.chart.graph && opts.chart.graph.setHover && opts.chart.graph.setHover(picked);

            var pt = picked.point;
            var seriesId = picked.seriesId;
            var seriesInd = opts.data.yIds[seriesId];
            var yItem = opts.data.ys[seriesInd];
            var color = pt.color || (this.isDarkMode ? yItem.colors_n[2] : yItem.colors_d[2]);

            this.labels.forEach(function (item) {
                item.$row.style.display = 'none';
            });
            if (this.allLabel) this.allLabel.$row.style.display = 'none';
            if (this.pieLabel) this.pieLabel.$row.style.display = 'none';

            // show the label row of the picked series
            var row = this.labels[seriesInd];
            row.$row.style.display = '';

            // use existing text nodes
            row.$labelText.nodeValue = pt.label || yItem.label;

            var yFmt = units.TUtils.getFormatter('yTooltipFormatter', opts.data, 0);
            var yVal = pt.y;
            ...
            // scatter mode: time vs number (from scatter.xType)
            var xType = (opts.data.scatter && opts.data.scatter.xType) ? opts.data.scatter.xType : 'time';

            if (xType === 'time') {
                row.$valueText.nodeValue = '(Y: ' + yStr + ')';
            } else {
                var xFmt = units.TUtils.getFormatter('xTooltipFormatter', opts.data, 0);
                var xVal = pt.x;
                if (typeof xVal === 'number' && isFinite(xVal)) {
                    xVal = Math.round(xVal);
                }
                var xStr = xFmt ? xFmt(xVal) : String(xVal);
                row.$valueText.nodeValue = '(X: ' + xStr + ', Y: ' + yStr + ')';
            }

            row.$value.style.color = color;

            // avoid dom flood
            this._scatterExtraRows = this._scatterExtraRows || [];

            var ensureExtraRow = function (idx) {
                var rowEl = this._scatterExtraRows[idx];
                if (rowEl) return rowEl;

                rowEl = document.createElement('div');
                rowEl.className = 'tchart--tip-row';
                ...
                vDiv.style.fontWeight = 'normal';

                rowEl.appendChild(kSpan);
                rowEl.appendChild(vDiv);

                this.$tipScroller.appendChild(rowEl);
                this._scatterExtraRows[idx] = rowEl;
                return rowEl;
            }.bind(this);

            var setExtraRow = function (idx, key, val) {
                var rowEl = ensureExtraRow(idx);
                rowEl.style.display = '';
            ...
            var hoverCfg = (opts.data.scatter && opts.data.scatter.hover) ? opts.data.scatter.hover : {};
            var maxKeys = hoverCfg.maxMetaKeys != null ? hoverCfg.maxMetaKeys : 12;
            var maxValLen = hoverCfg.maxMetaValueLen != null ? hoverCfg.maxMetaValueLen : 120;

            var used = 0;

            // size
            if (pt.sizePx != null) {
                setExtraRow(used++, 'Size:', String(pt.sizePx));
            }

            // meta
            if (pt.meta) {
                var cnt = 0;
                for (var key in pt.meta) {
                    if (cnt++ >= maxKeys) break;

                    var val = pt.meta[key];
                    if (typeof val === 'object') val = JSON.stringify(val);
                    val = String(val);

                    if (val.length > maxValLen) val = val.substr(0, maxValLen) + '...';

                    setExtraRow(used++, key + ':', val);
                }
            }

            // hide unused pooled rows
            for (var ri = used; ri < this._scatterExtraRows.length; ri++) {
                this._scatterExtraRows[ri].style.display = 'none';
            }
            ...
            var xType = (opts.data.scatter && opts.data.scatter.xType) ? opts.data.scatter.xType : 'time';
            var xValHeader = pt.x;
            if (xType !== 'time' && typeof xValHeader === 'number' && isFinite(xValHeader)) {
                xValHeader = Math.round(xValHeader);
            }

            this.$tipDtText.nodeValue = xFmt(xValHeader);
            ...
            // scatter tooltip
            this.$tip.classList.add('tchart--tip__scatter');
            this.$tip.style.width = 'auto';

            this.tipH = this.$tip.offsetHeight;
            this.tipW = this.$tip.offsetWidth;

            // grow tooltip to fit, but clamp
            var desired = 0;
            desired = Math.max(desired, this.$tipDt.scrollWidth + 24);

            for (var ci = 0; ci < this.$tipScroller.children.length; ci++) {
                var rowEl = this.$tipScroller.children[ci];
                desired = Math.max(desired, rowEl.scrollWidth + 24);
            }

            var minW = 220;
            var maxW = Math.min(520, dims.w - pLeft - pRight);
            desired = Math.max(minW, Math.min(desired, maxW));

            this.$tip.style.width = desired + 'px';

            this.tipH = this.$tip.offsetHeight;
            this.tipW = this.$tip.offsetWidth;

            var left = this.dx + 20;
            if (this.dx > dims.w / 2) left = this.dx - this.tipW - 20;

            var top = this.dy - this.tipH / 2 - dims.t;
            top = Math.min(Math.max(top, pTop), dims.h - this.tipH - pBottom);

            left = Math.min(Math.max(left, pLeft / 2), dims.w - this.tipW - pRight / 2);

            this.$tip.style.transform = 'translate(' + (left << 0) + 'px,' + (top + dims.t << 0) + 'px)';
            this.$tip.style.webkitTransform = 'translate(' + (left << 0) + 'px,' + (top + dims.t << 0) + 'px)';

            this.updateTipScrollClasses();

            this.prevXInd = -999;
            this.lastScatterPoint = pt;
        },
...
(function () {
    ...
    var TUtils = {
        ...
        xRangeFormatter: function (x, isZoom) {
            var dt = new Date(x);
            var dtc = [dt.getUTCFullYear(), dt.getUTCMonth(), dt.getUTCDate()];
        ...
        niceNum: function (range, round) {
            ...
            if (round) {
                // Round to nearest: 1, 2, 2.5, 4, 5, 10
        ...
        /**
         * Calculate nice Y-axis range and step
         */
        niceScale: function (yMin, yMax, maxTicks) {
            maxTicks = maxTicks || 5;
            ...
            var range = yMax - yMin;
            var step = this.niceNum(range / (maxTicks - 1), true);

            var niceMin = Math.floor(yMin / step) * step;
            var niceMax = Math.ceil(yMax / step) * step;

            // Safety: ensure niceMax >= yMax
            if (niceMax < yMax) {
                niceMax += step;
            }
            ...
            // Fix floating point
            var p = 1e12;
            niceMin = Math.round(niceMin * p) / p;
            niceMax = Math.round(niceMax * p) / p;
            step = Math.round(step * p) / p;
        ...
        yTickFormatter: function (val, step, isFractional) {
            ...
            var decimals = 0;
            if (step > 0 && step < 1) {
                decimals = Math.ceil(-Math.log10(step) + 0.0001);
        ...
        roundRange: function (y1, y2, cnt, refRange) {
            ...
            var nice = this.niceScale(y1, y2, Math.floor(cnt) + 1);
...
(function () {
    ...
    var TScatter = function (opts) {
        this.opts = opts;
        this.ctx = opts.ctx;

        // hover highlight overlay (main scatter only)
    ...
    TScatter.prototype = {
        // z_mode means
        // manual: point.z, else series.z, else 0
        // auto: smaller dots always on top of larger dots, then z
        _getZMode: function () {
            var s = this.opts.data.scatter;
            return (s && (s.z_mode === 'manual' || s.z_mode === 'auto')) ? s.z_mode : 'auto';
        },

        _getPointZManual: function (series, p) {
            if (p && p.z != null && !isNaN(p.z)) return +p.z;
            if (series && series.z != null && !isNaN(series.z)) return +series.z;
            return 0;
        },

        _getPointSizePx: function (seriesCfg, p, scatterCfg) {
            var defSize = (scatterCfg.size && scatterCfg.size.defaultPx) ? scatterCfg.size.defaultPx : 3;
            var sp = (p && p.sizePx != null) ? p.sizePx : defSize;
            sp = +sp;
            if (!isFinite(sp) || sp <= 0) sp = defSize;
            return sp;
        },

        _getPointZKey: function (seriesCfg, p, scatterCfg) {
            var zMode = this._getZMode();
            var sizePx = this._getPointSizePx(seriesCfg, p, scatterCfg);
            var zManual = this._getPointZManual(seriesCfg, p);
        ...
        // 1 if a is on top of b, -1 if below, 0 if equal
        _cmpOnTop: function (a, b) {
            if (!a) return -1;
            if (!b) return 1;
        ...
        // sort for drawing bottom -> top
        _cmpDrawOrder: function (a, b) {
            var zMode = a.zMode;

            if (zMode === 'manual') {
                if (a.z !== b.z) return a.z - b.z; // low first
                if (a.sizePx !== b.sizePx) return b.sizePx - a.sizePx; // bigger first
        ...
        _resolveBorderColor: function (seriesCfg, p) {
            //  dot > series > scatter
            var scatterCfg = this.opts.data.scatter;

            var v;
            if (p && ('border_color' in p)) {
                v = p.border_color;
            } else if (seriesCfg && ('border_color' in seriesCfg)) {
                v = seriesCfg.border_color;
            } else if (scatterCfg && ('border_color' in scatterCfg)) {
                v = scatterCfg.border_color;
            } else {
                v = undefined;
            }
        ...
        clearHover: function () {
            if (this.$hoverPoint) {
                this.$hoverPoint.style.display = 'none';
            }
            this._hovered = null;
        },

        setHover: function (picked) {
            if (!this.$hoverPoint) return;

            if (!picked || !picked._rendered) {
                this.clearHover();
                return;
            }

            if (this._hovered === picked._rendered) return;
            this._hovered = picked._rendered;

            var r = picked._rendered.rDev;
            var opts = this.opts;
            var dpi = opts.settings.dpi;
            var dimsGraph = opts.state.dims.graph;
            ...
            // hack for old browsers that lose sync with dom dimensions
            var dimsComposer = opts.state.dims.composer;
            var bounds = opts.$parent.getBoundingClientRect();
            var scaleX = bounds.width / dimsComposer.w;
            var scaleY = bounds.height / dimsComposer.h;

            var cssX = ((((dimsGraph.l * dpi) | 0) + picked._rendered.cxDev) / dpi) * scaleX - cssR;
            var cssY = ((((dimsGraph.t * dpi) | 0) + picked._rendered.cyDev) / dpi) * scaleY - cssR;

            this.$hoverPoint.style.width = cssDia + 'px';
            this.$hoverPoint.style.height = cssDia + 'px';
            this.$hoverPoint.style.transform = 'translate(' + cssX + 'px,' + cssY + 'px)';
            this.$hoverPoint.style.backgroundColor = picked._rendered.hoverColor || '#ffd400';
            this.$hoverPoint.style.display = 'block';
        },

        init: function () {
            var data = this.opts.data.scatter;
            if (!data) return;

            var BLOCK_SIZE = 1000;
            for (var sid in data.series) {
                var s = data.series[sid];
                if (!s.points || !s.points.length) continue;

                s.blocks = [];
                for (var i = 0; i < s.points.length; i += BLOCK_SIZE) {
                    var chunk = s.points.slice(i, i + BLOCK_SIZE);
                    var yMin = Number.MAX_VALUE;
                    var yMax = -Number.MAX_VALUE;
                    for (var k = 0; k < chunk.length; k++) {
                        if (chunk[k].y < yMin) yMin = chunk[k].y;
                        if (chunk[k].y > yMax) yMax = chunk[k].y;
                    }
                    s.blocks.push({
                        xMin: chunk[0].x,
                        xMax: chunk[chunk.length - 1].x,
                        yMin: yMin,
                        yMax: yMax,
                        points: chunk
                    });
            ...
            this.miniBins = {};
            var xgMin = this.opts.state.xgMin;
            var xgRange = this.opts.state.xgMax - xgMin;
            var BIN_COUNT = 300;

            if (xgRange > 0) {
                for (var sid in data.series) {
                    var s = data.series[sid];
                    this.miniBins[sid] = new Uint16Array(BIN_COUNT);
                    var pts = s.points;
                    for (var i = 0; i < pts.length; i++) {
                        var bin = Math.floor((pts[i].x - xgMin) / xgRange * (BIN_COUNT - 1));
                        if (bin >= 0 && bin < BIN_COUNT) {
                            if (this.miniBins[sid][bin] < 65535) this.miniBins[sid][bin]++;
                        }
                    }
                }
            }
        ...
        render: function () {
            var opts = this.opts;
            var mini = opts.additional.mini;
            var ctx = mini ? this.ctx : opts.ctx;
        ...
        renderMini: function (ctx) {
            var state = this.opts.state;
            var dims = state.dims.mini;
            var dpi = this.opts.settings.dpi;

            var BIN_COUNT = 300;

            // use float so we can weight by animated opacity om_
            var combined = new Float32Array(BIN_COUNT);
            var maxC = 0;

            for (var sid in this.miniBins) {
                var ind = this.opts.data.yIds[sid];
                if (ind == null) continue;

                var o = state['om_' + ind];
                if (!(o > 0)) continue;

                var bins = this.miniBins[sid];
                for (var i = 0; i < BIN_COUNT; i++) {
                    var v = bins[i] * o;
                    combined[i] += v;
                    if (combined[i] > maxC) maxC = combined[i];
                }
            }

            if (maxC <= 0) return;

            ctx.fillStyle = this.isDarkMode ? 'rgba(255, 255, 255, 0.4)' : 'rgba(0, 0, 0, 0.3)';

            var binW = (dims.w * dpi) / BIN_COUNT;
            var h = dims.h * dpi;
            var xOff = dims.l * dpi;
            var yOff = dims.t * dpi;
            ...
            for (var i = 0; i < BIN_COUNT; i++) {
                ...
                if (c > 0) {
                    var bh = (c / maxC) * h;
                    ctx.fillRect(xOff + i * binW, yOff + h - bh, Math.max(binW, 1), bh);
                }
            }
        },
        ...
        renderMain: function (ctx) {
            ...
            var dims = state.dims.graph;
            var dpi = opts.settings.dpi;
            var sData = opts.data.scatter;

            if (!sData || !sData.series) return;

            var padd = opts.settings.PADD;
            var pTop = padd[0];
            var pRight = padd[1];
            var pBottom = padd[2];
            var pLeft = padd[3];
            ...
            var plotW = Math.max(dims.w - pLeft - pRight, 1);
            var plotH = Math.max(dims.h - pTop - pBottom, 1);

            var xScale = (plotW * dpi) / (x2 - x1);
            var yScale = (plotH * dpi) / (y2 - y1);

            // render grid for decimation (one final point per render cell)
            var gridCellCss = (sData.hover && sData.hover.gridCellPx) || 3;
            var gridCellDev = Math.max(1, (gridCellCss * dpi) | 0);

            var wDev = (dims.w * dpi) | 0;
            var hDev = (dims.h * dpi) | 0;
            var gridW = Math.max(1, (wDev / gridCellDev) | 0);

            var xMinDev = (pLeft * dpi) | 0;
            var xMaxDev = ((dims.w - pRight) * dpi) | 0;
            var yMinDev = (pTop * dpi) | 0;
            var yMaxDev = ((dims.h - pBottom) * dpi) | 0;

            var maxPts = (sData.hover && sData.hover.maxRenderPoints) || 60000;

            var cellMap = Object.create(null);
            var cellKeys = [];

            var seq = 0;
            var stopNewCells = false;

            for (var sid in sData.series) {
                var ind = opts.data.yIds[sid];
                if (ind == null) continue;
                ...
                if (!state['e_' + ind] && o === 0) continue;

                var seriesCfg = sData.series[sid];
                if (!seriesCfg || !seriesCfg.points || !seriesCfg.points.length) continue;

                var blocks = seriesCfg.blocks || [{
                    points: seriesCfg.points,
                    xMin: seriesCfg.xMin,
                    xMax: seriesCfg.xMax
                }];

                var yItem = opts.data.ys[ind];
                var seriesColor = this.isDarkMode ? yItem.colors_n[0] : yItem.colors_d[0];

                for (var b = 0; b < blocks.length; b++) {
                    var blk = blocks[b];
                    if (!blk) continue;

                    if (blk.xMax < x1) continue;
                    if (blk.xMin > x2) break; // Blocks are sorted by xMin
                    ...
                    for (var i = start; i < pts.length; i++) {
                        ...
                        // Y check is still needed
                        if (p.y < y1 || p.y > y2) continue;

                        var cxDev = (xMinDev + (p.x - x1) * xScale) | 0;
                        var cyDev = (yMaxDev - (p.y - y1) * yScale) | 0;

                        if (cxDev < xMinDev || cxDev > xMaxDev || cyDev < yMinDev || cyDev > yMaxDev) continue;

                        var gx = (cxDev / gridCellDev) | 0;
                        var gy = (cyDev / gridCellDev) | 0;
                        var key = gy * gridW + gx;

                        // z_mode is constant per render
                        var curSeq = seq++;
                        var sizePx = this._getPointSizePx(seriesCfg, p, sData);
                        var rDev = (sizePx * dpi) / 2;
                        var zVal = this._getPointZManual(seriesCfg, p);
                        var zMode = this._getZMode();

                        var prev = cellMap[key];
                        var shouldStore = false;

                        if (!prev) {
                            shouldStore = !stopNewCells;
                        } else {
                            if (zMode === 'manual') {
                                if (zVal !== prev.z) shouldStore = zVal > prev.z;
                                else if (sizePx !== prev.sizePx) shouldStore = sizePx < prev.sizePx;
                            } else {
                                // auto
                                if (sizePx !== prev.sizePx) shouldStore = sizePx < prev.sizePx;
                                else if (zVal !== prev.z) shouldStore = zVal > prev.z;
                            }
                        }

                        if (shouldStore) {
                            var candidate = {
                                // _id removed: reference equality used instead
                                _seq: curSeq,
                                p: p,
                                sid: sid,
                                seriesIndex: ind,
                                ...
                                hoverColor: this._resolveHoverColor(seriesCfg, p),
                                borderColor: this._resolveBorderColor(seriesCfg, p),

                                // used by z comparisons
                                zMode: zMode,
                                z: zVal,
                                sizePx: sizePx,
                            ...
                            if (!prev) {
                                cellMap[key] = candidate;
                                cellKeys.push(key);
                                if (cellKeys.length >= maxPts) stopNewCells = true;
                            } else {
                                cellMap[key] = candidate;
                            }
                        }
            ...
            var drawList = new Array(cellKeys.length);
            var maxRDev = 0;
            for (var k = 0; k < cellKeys.length; k++) {
                var it = cellMap[cellKeys[k]];
                drawList[k] = it;
                if (it && it.rDev > maxRDev) maxRDev = it.rDev;
            }

            // draw bottom -> top (z_mode)
            drawList.sort(this._cmpDrawOrder.bind(this));

            // pick grid and draw
            var pickCellCss = (sData.hover && sData.hover.pickCellPx) || 10;
            var pickCellDev = Math.max(1, (pickCellCss * dpi) | 0);
            var pickGridW = Math.max(1, (wDev / pickCellDev) | 0);
            var pickGrid = Object.create(null);

            var baseX = (dims.l * dpi) | 0;
            var baseY = (dims.t * dpi) | 0;
            var borderW = Math.max(1, dpi);
            ...
            for (var di = 0; di < drawList.length; di++) {
                ...
                // pick grid
                var pgx = (dpt.cxDev / pickCellDev) | 0;
                var pgy = (dpt.cyDev / pickCellDev) | 0;
                var pkey = pgy * pickGridW + pgx;
                var bucket = pickGrid[pkey];
                if (!bucket) pickGrid[pkey] = bucket = [];
                bucket.push(dpt);

                // draw
                if (dpt.alpha > 0) {
                    ctx.globalAlpha = dpt.alpha;
                    ctx.beginPath();
                    ctx.arc(baseX + dpt.cxDev, baseY + dpt.cyDev, dpt.rDev, 0, 2 * Math.PI);
                    ctx.fillStyle = dpt.fill;
                    ctx.fill();

                    if (dpt.borderColor) {
                        ctx.lineWidth = borderW;
                        ctx.strokeStyle = dpt.borderColor;
                        ctx.stroke();
                    }
                }
            }
            ...
            this.lastGridMeta = {
                ...
                gridW: pickGridW,
                maxRDev: maxRDev,
                hitPadDev: (((sData.hover && sData.hover.hitPaddingPx) || 3) * dpi)
            };
        },

        pickPoint: function (sx, sy) {
            // sx, sy are GRAPH-LOCAL CSS pixels (0..dims.w, 0..dims.h)
            if (!this.lastGrid || !this.lastGridMeta) return null;

            var dpi = this.opts.settings.dpi;
            var meta = this.lastGridMeta;

            var cxDev = (sx * dpi) | 0;
            var cyDev = (sy * dpi) | 0;

            var cell = meta.cell;
            var gridW = meta.gridW || ((meta.wDev / cell) | 0);

            var gx = (cxDev / cell) | 0;
            var gy = (cyDev / cell) | 0;

            var radiusCells = Math.max(2, Math.ceil((meta.maxRDev + meta.hitPadDev) / cell) + 1);
            ...
            for (var dy = -radiusCells; dy <= radiusCells; dy++) {
                ...
                for (var dx = -radiusCells; dx <= radiusCells; dx++) {
                    ...
                    var key = yy * gridW + xx;
                    var bucket = this.lastGrid[key];
                    if (!bucket) continue;

                    for (var bi = 0; bi < bucket.length; bi++) {
                        var item = bucket[bi];

                        var ddx = item.cxDev - cxDev;
                        var ddy = item.cyDev - cyDev;
                        var dist = ddx * ddx + ddy * ddy;

                        var hitR = item.rDev + meta.hitPadDev;
                        if (dist > hitR * hitR) continue;

                        if (!best) {
                            best = item;
                            bestDist = dist;
                        } else {
                            // prefer top-most by z_mode, then nearer
                            var cmp = this._cmpOnTop(item, best);
                            if (cmp > 0 || (cmp === 0 && dist < bestDist)) {
                                best = item;
                                bestDist = dist;
                            }
                        }
